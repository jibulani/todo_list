...
function addTodoFromTemplate(todo) {
    ...
    var date = todo.date.getDate();
    if (date < 10) {
        date = '0' + date;
    }
    if ((todo.date.getMonth() + 1) < 10) {
        var month = '0' + (todo.date.getMonth() + 1);
    }
    var year = todo.date.getFullYear() % 100;
    if (year < 10) {
        year = '0' + year;
    }
    var hours = todo.date.getHours();
    if (hours <  10) {
        hours = '0' + hours;
    }
    var minutes = todo.date.getMinutes();
    if (minutes <  10) {
        minutes = '0' + minutes;
    }
    var seconds = todo.date.getSeconds();
    if (seconds <  10) {
        seconds = '0' + seconds;
    }
    newElement.querySelector('.task__time').textContent = hours + ":" + minutes + ":" + seconds;
    newElement.querySelector('.task__date').textContent = date + "/" + month + "/" + year;
...
function Statistic() {
    ...
    this.statsDonelElement = this.statsElement.querySelector('.statistic__done');
    this.statsTodoElement = this.statsElement.querySelector('.statistic__left');
    this.statsTotalElement = this.statsElement.querySelector('.statistic__total');
}
